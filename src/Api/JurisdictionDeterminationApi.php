<?php
/**
 * JurisdictionDeterminationApi
 * PHP version 5
 *
 * @category Class
 * @package  PhoneBurner\AvalaraAFC
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */

/**
 * SaasPro
 *
 * APIs to interface with communications tax engine.<br />The API requires Basic authentication.<br />Users with access to multiple clients must also set request header parameter for <code>client_id</code>.<br />Set <code>client_profile_id</code> to specify profile to be used for taxation.<br /><br />Effective January 1st, 2022 all Avalara products will be enforcing TLS 1.2 server-side. Please ensure that your implementation sets a minimal of TLS 1.2 encryption when making web requests to Avalara APIs.
 *
 * The version of the OpenAPI document: v2
 * 
 * Generated by: https://openapi-generator.tech
 * OpenAPI Generator version: 4.3.1
 */

/**
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

namespace PhoneBurner\AvalaraAFC\Api;

use GuzzleHttp\Client;
use GuzzleHttp\ClientInterface;
use GuzzleHttp\Exception\RequestException;
use GuzzleHttp\Psr7\MultipartStream;
use GuzzleHttp\Psr7\Request;
use GuzzleHttp\RequestOptions;
use PhoneBurner\AvalaraAFC\ApiException;
use PhoneBurner\AvalaraAFC\Configuration;
use PhoneBurner\AvalaraAFC\HeaderSelector;
use PhoneBurner\AvalaraAFC\ObjectSerializer;

/**
 * JurisdictionDeterminationApi Class Doc Comment
 *
 * @category Class
 * @package  PhoneBurner\AvalaraAFC
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */
class JurisdictionDeterminationApi
{
    /**
     * @var ClientInterface
     */
    protected $client;

    /**
     * @var Configuration
     */
    protected $config;

    /**
     * @var HeaderSelector
     */
    protected $headerSelector;

    /**
     * @var int Host index
     */
    protected $hostIndex;

    /**
     * @param ClientInterface $client
     * @param Configuration   $config
     * @param HeaderSelector  $selector
     * @param int             $host_index (Optional) host index to select the list of hosts if defined in the OpenAPI spec
     */
    public function __construct(
        ClientInterface $client = null,
        Configuration $config = null,
        HeaderSelector $selector = null,
        $host_index = 0
    ) {
        $this->client = $client ?: new Client();
        $this->config = $config ?: new Configuration();
        $this->headerSelector = $selector ?: new HeaderSelector();
        $this->hostIndex = $host_index;
    }

    /**
     * Set the host index
     *
     * @param  int Host index (required)
     */
    public function setHostIndex($host_index)
    {
        $this->hostIndex = $host_index;
    }

    /**
     * Get the host index
     *
     * @return Host index
     */
    public function getHostIndex()
    {
        return $this->hostIndex;
    }

    /**
     * @return Configuration
     */
    public function getConfig()
    {
        return $this->config;
    }

    /**
     * Operation apiV2AfcPCodePost
     *
     * Get PCode(s) associated with a location - Ctry/State/County/City/Zip/NpaNxx/Fips.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\PCodeLookupRequest $p_code_lookup_request p_code_lookup_request (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \PhoneBurner\AvalaraAFC\Model\PCodeLookupResult
     */
    public function apiV2AfcPCodePost($p_code_lookup_request = null)
    {
        list($response) = $this->apiV2AfcPCodePostWithHttpInfo($p_code_lookup_request);
        return $response;
    }

    /**
     * Operation apiV2AfcPCodePostWithHttpInfo
     *
     * Get PCode(s) associated with a location - Ctry/State/County/City/Zip/NpaNxx/Fips.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\PCodeLookupRequest $p_code_lookup_request (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \PhoneBurner\AvalaraAFC\Model\PCodeLookupResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function apiV2AfcPCodePostWithHttpInfo($p_code_lookup_request = null)
    {
        $request = $this->apiV2AfcPCodePostRequest($p_code_lookup_request);

        try {
            $options = $this->createHttpClientOption();
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
                    $e->getResponse() ? $e->getResponse()->getHeaders() : null,
                    $e->getResponse() ? (string) $e->getResponse()->getBody() : null
                );
            }

            $statusCode = $response->getStatusCode();

            if ($statusCode < 200 || $statusCode > 299) {
                throw new ApiException(
                    sprintf(
                        '[%d] Error connecting to the API (%s)',
                        $statusCode,
                        $request->getUri()
                    ),
                    $statusCode,
                    $response->getHeaders(),
                    $response->getBody()
                );
            }

            $responseBody = $response->getBody();
            switch($statusCode) {
                case 200:
                    if ('\PhoneBurner\AvalaraAFC\Model\PCodeLookupResult' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\PhoneBurner\AvalaraAFC\Model\PCodeLookupResult', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\PhoneBurner\AvalaraAFC\Model\PCodeLookupResult';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
                $content = (string) $responseBody;
            }

            return [
                ObjectSerializer::deserialize($content, $returnType, []),
                $response->getStatusCode(),
                $response->getHeaders()
            ];

        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\PhoneBurner\AvalaraAFC\Model\PCodeLookupResult',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
            }
            throw $e;
        }
    }

    /**
     * Operation apiV2AfcPCodePostAsync
     *
     * Get PCode(s) associated with a location - Ctry/State/County/City/Zip/NpaNxx/Fips.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\PCodeLookupRequest $p_code_lookup_request (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2AfcPCodePostAsync($p_code_lookup_request = null)
    {
        return $this->apiV2AfcPCodePostAsyncWithHttpInfo($p_code_lookup_request)
            ->then(
                function ($response) {
                    return $response[0];
                }
            );
    }

    /**
     * Operation apiV2AfcPCodePostAsyncWithHttpInfo
     *
     * Get PCode(s) associated with a location - Ctry/State/County/City/Zip/NpaNxx/Fips.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\PCodeLookupRequest $p_code_lookup_request (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2AfcPCodePostAsyncWithHttpInfo($p_code_lookup_request = null)
    {
        $returnType = '\PhoneBurner\AvalaraAFC\Model\PCodeLookupResult';
        $request = $this->apiV2AfcPCodePostRequest($p_code_lookup_request);

        return $this->client
            ->sendAsync($request, $this->createHttpClientOption())
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, $returnType, []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                },
                function ($exception) {
                    $response = $exception->getResponse();
                    $statusCode = $response->getStatusCode();
                    throw new ApiException(
                        sprintf(
                            '[%d] Error connecting to the API (%s)',
                            $statusCode,
                            $exception->getRequest()->getUri()
                        ),
                        $statusCode,
                        $response->getHeaders(),
                        $response->getBody()
                    );
                }
            );
    }

    /**
     * Create request for operation 'apiV2AfcPCodePost'
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\PCodeLookupRequest $p_code_lookup_request (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function apiV2AfcPCodePostRequest($p_code_lookup_request = null)
    {

        $resourcePath = '/api/v2/afc/PCode';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        $multipart = false;




        // body params
        $_tempBody = null;
        if (isset($p_code_lookup_request)) {
            $_tempBody = $p_code_lookup_request;
        }

        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(
                ['text/plain', 'application/json', 'text/json']
            );
        } else {
            $headers = $this->headerSelector->selectHeaders(
                ['text/plain', 'application/json', 'text/json'],
                ['application/json-patch+json', 'application/json', 'text/json', 'application/_*+json']
            );
        }

        // for model (json/xml)
        if (isset($_tempBody)) {
            // $_tempBody is the method argument, if present
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($_tempBody));
            } else {
                $httpBody = $_tempBody;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    $multipartContents[] = [
                        'name' => $formParamName,
                        'contents' => $formParamValue
                    ];
                }
                // for HTTP post (form)
                $httpBody = new MultipartStream($multipartContents);

            } elseif ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \GuzzleHttp\Psr7\build_query($formParams);
            }
        }


        $defaultHeaders = [];
        if ($this->config->getUserAgent()) {
            $defaultHeaders['User-Agent'] = $this->config->getUserAgent();
        }

        $headers = array_merge(
            $defaultHeaders,
            $headerParams,
            $headers
        );

        $query = \GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'POST',
            $this->config->getHost() . $resourcePath . ($query ? "?{$query}" : ''),
            $headers,
            $httpBody
        );
    }

    /**
     * Operation apiV2GeoBatchLogProcessIdGet
     *
     * Retrieves log on Geo Batch file
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \PhoneBurner\AvalaraAFC\Model\GeoBatchLog
     */
    public function apiV2GeoBatchLogProcessIdGet($process_id)
    {
        list($response) = $this->apiV2GeoBatchLogProcessIdGetWithHttpInfo($process_id);
        return $response;
    }

    /**
     * Operation apiV2GeoBatchLogProcessIdGetWithHttpInfo
     *
     * Retrieves log on Geo Batch file
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \PhoneBurner\AvalaraAFC\Model\GeoBatchLog, HTTP status code, HTTP response headers (array of strings)
     */
    public function apiV2GeoBatchLogProcessIdGetWithHttpInfo($process_id)
    {
        $request = $this->apiV2GeoBatchLogProcessIdGetRequest($process_id);

        try {
            $options = $this->createHttpClientOption();
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
                    $e->getResponse() ? $e->getResponse()->getHeaders() : null,
                    $e->getResponse() ? (string) $e->getResponse()->getBody() : null
                );
            }

            $statusCode = $response->getStatusCode();

            if ($statusCode < 200 || $statusCode > 299) {
                throw new ApiException(
                    sprintf(
                        '[%d] Error connecting to the API (%s)',
                        $statusCode,
                        $request->getUri()
                    ),
                    $statusCode,
                    $response->getHeaders(),
                    $response->getBody()
                );
            }

            $responseBody = $response->getBody();
            switch($statusCode) {
                case 200:
                    if ('\PhoneBurner\AvalaraAFC\Model\GeoBatchLog' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\PhoneBurner\AvalaraAFC\Model\GeoBatchLog', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchLog';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
                $content = (string) $responseBody;
            }

            return [
                ObjectSerializer::deserialize($content, $returnType, []),
                $response->getStatusCode(),
                $response->getHeaders()
            ];

        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\PhoneBurner\AvalaraAFC\Model\GeoBatchLog',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
            }
            throw $e;
        }
    }

    /**
     * Operation apiV2GeoBatchLogProcessIdGetAsync
     *
     * Retrieves log on Geo Batch file
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchLogProcessIdGetAsync($process_id)
    {
        return $this->apiV2GeoBatchLogProcessIdGetAsyncWithHttpInfo($process_id)
            ->then(
                function ($response) {
                    return $response[0];
                }
            );
    }

    /**
     * Operation apiV2GeoBatchLogProcessIdGetAsyncWithHttpInfo
     *
     * Retrieves log on Geo Batch file
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchLogProcessIdGetAsyncWithHttpInfo($process_id)
    {
        $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchLog';
        $request = $this->apiV2GeoBatchLogProcessIdGetRequest($process_id);

        return $this->client
            ->sendAsync($request, $this->createHttpClientOption())
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, $returnType, []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                },
                function ($exception) {
                    $response = $exception->getResponse();
                    $statusCode = $response->getStatusCode();
                    throw new ApiException(
                        sprintf(
                            '[%d] Error connecting to the API (%s)',
                            $statusCode,
                            $exception->getRequest()->getUri()
                        ),
                        $statusCode,
                        $response->getHeaders(),
                        $response->getBody()
                    );
                }
            );
    }

    /**
     * Create request for operation 'apiV2GeoBatchLogProcessIdGet'
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function apiV2GeoBatchLogProcessIdGetRequest($process_id)
    {
        // verify the required parameter 'process_id' is set
        if ($process_id === null || (is_array($process_id) && count($process_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $process_id when calling apiV2GeoBatchLogProcessIdGet'
            );
        }

        $resourcePath = '/api/v2/geo/batch/log/{processId}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        $multipart = false;



        // path params
        if ($process_id !== null) {
            $resourcePath = str_replace(
                '{' . 'processId' . '}',
                ObjectSerializer::toPathValue($process_id),
                $resourcePath
            );
        }

        // body params
        $_tempBody = null;

        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(
                ['text/plain', 'application/json', 'text/json']
            );
        } else {
            $headers = $this->headerSelector->selectHeaders(
                ['text/plain', 'application/json', 'text/json'],
                []
            );
        }

        // for model (json/xml)
        if (isset($_tempBody)) {
            // $_tempBody is the method argument, if present
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($_tempBody));
            } else {
                $httpBody = $_tempBody;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    $multipartContents[] = [
                        'name' => $formParamName,
                        'contents' => $formParamValue
                    ];
                }
                // for HTTP post (form)
                $httpBody = new MultipartStream($multipartContents);

            } elseif ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \GuzzleHttp\Psr7\build_query($formParams);
            }
        }


        $defaultHeaders = [];
        if ($this->config->getUserAgent()) {
            $defaultHeaders['User-Agent'] = $this->config->getUserAgent();
        }

        $headers = array_merge(
            $defaultHeaders,
            $headerParams,
            $headers
        );

        $query = \GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'GET',
            $this->config->getHost() . $resourcePath . ($query ? "?{$query}" : ''),
            $headers,
            $httpBody
        );
    }

    /**
     * Operation apiV2GeoBatchStatusProcessIdGet
     *
     * Retrieves information on Geo Batch file status
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \PhoneBurner\AvalaraAFC\Model\GeoBatchStatus
     */
    public function apiV2GeoBatchStatusProcessIdGet($process_id)
    {
        list($response) = $this->apiV2GeoBatchStatusProcessIdGetWithHttpInfo($process_id);
        return $response;
    }

    /**
     * Operation apiV2GeoBatchStatusProcessIdGetWithHttpInfo
     *
     * Retrieves information on Geo Batch file status
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \PhoneBurner\AvalaraAFC\Model\GeoBatchStatus, HTTP status code, HTTP response headers (array of strings)
     */
    public function apiV2GeoBatchStatusProcessIdGetWithHttpInfo($process_id)
    {
        $request = $this->apiV2GeoBatchStatusProcessIdGetRequest($process_id);

        try {
            $options = $this->createHttpClientOption();
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
                    $e->getResponse() ? $e->getResponse()->getHeaders() : null,
                    $e->getResponse() ? (string) $e->getResponse()->getBody() : null
                );
            }

            $statusCode = $response->getStatusCode();

            if ($statusCode < 200 || $statusCode > 299) {
                throw new ApiException(
                    sprintf(
                        '[%d] Error connecting to the API (%s)',
                        $statusCode,
                        $request->getUri()
                    ),
                    $statusCode,
                    $response->getHeaders(),
                    $response->getBody()
                );
            }

            $responseBody = $response->getBody();
            switch($statusCode) {
                case 200:
                    if ('\PhoneBurner\AvalaraAFC\Model\GeoBatchStatus' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\PhoneBurner\AvalaraAFC\Model\GeoBatchStatus', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchStatus';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
                $content = (string) $responseBody;
            }

            return [
                ObjectSerializer::deserialize($content, $returnType, []),
                $response->getStatusCode(),
                $response->getHeaders()
            ];

        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\PhoneBurner\AvalaraAFC\Model\GeoBatchStatus',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
            }
            throw $e;
        }
    }

    /**
     * Operation apiV2GeoBatchStatusProcessIdGetAsync
     *
     * Retrieves information on Geo Batch file status
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchStatusProcessIdGetAsync($process_id)
    {
        return $this->apiV2GeoBatchStatusProcessIdGetAsyncWithHttpInfo($process_id)
            ->then(
                function ($response) {
                    return $response[0];
                }
            );
    }

    /**
     * Operation apiV2GeoBatchStatusProcessIdGetAsyncWithHttpInfo
     *
     * Retrieves information on Geo Batch file status
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchStatusProcessIdGetAsyncWithHttpInfo($process_id)
    {
        $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchStatus';
        $request = $this->apiV2GeoBatchStatusProcessIdGetRequest($process_id);

        return $this->client
            ->sendAsync($request, $this->createHttpClientOption())
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, $returnType, []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                },
                function ($exception) {
                    $response = $exception->getResponse();
                    $statusCode = $response->getStatusCode();
                    throw new ApiException(
                        sprintf(
                            '[%d] Error connecting to the API (%s)',
                            $statusCode,
                            $exception->getRequest()->getUri()
                        ),
                        $statusCode,
                        $response->getHeaders(),
                        $response->getBody()
                    );
                }
            );
    }

    /**
     * Create request for operation 'apiV2GeoBatchStatusProcessIdGet'
     *
     * @param  string $process_id Process Id for Geo Batch File. (required)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function apiV2GeoBatchStatusProcessIdGetRequest($process_id)
    {
        // verify the required parameter 'process_id' is set
        if ($process_id === null || (is_array($process_id) && count($process_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $process_id when calling apiV2GeoBatchStatusProcessIdGet'
            );
        }

        $resourcePath = '/api/v2/geo/batch/status/{processId}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        $multipart = false;



        // path params
        if ($process_id !== null) {
            $resourcePath = str_replace(
                '{' . 'processId' . '}',
                ObjectSerializer::toPathValue($process_id),
                $resourcePath
            );
        }

        // body params
        $_tempBody = null;

        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(
                ['text/plain', 'application/json', 'text/json']
            );
        } else {
            $headers = $this->headerSelector->selectHeaders(
                ['text/plain', 'application/json', 'text/json'],
                []
            );
        }

        // for model (json/xml)
        if (isset($_tempBody)) {
            // $_tempBody is the method argument, if present
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($_tempBody));
            } else {
                $httpBody = $_tempBody;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    $multipartContents[] = [
                        'name' => $formParamName,
                        'contents' => $formParamValue
                    ];
                }
                // for HTTP post (form)
                $httpBody = new MultipartStream($multipartContents);

            } elseif ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \GuzzleHttp\Psr7\build_query($formParams);
            }
        }


        $defaultHeaders = [];
        if ($this->config->getUserAgent()) {
            $defaultHeaders['User-Agent'] = $this->config->getUserAgent();
        }

        $headers = array_merge(
            $defaultHeaders,
            $headerParams,
            $headers
        );

        $query = \GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'GET',
            $this->config->getHost() . $resourcePath . ($query ? "?{$query}" : ''),
            $headers,
            $httpBody
        );
    }

    /**
     * Operation apiV2GeoBatchUploadPost
     *
     * Uploads file to Geo Batch.
     *
     * @param  \SplFileObject $geo_batch_file geo_batch_file (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse
     */
    public function apiV2GeoBatchUploadPost($geo_batch_file = null)
    {
        list($response) = $this->apiV2GeoBatchUploadPostWithHttpInfo($geo_batch_file);
        return $response;
    }

    /**
     * Operation apiV2GeoBatchUploadPostWithHttpInfo
     *
     * Uploads file to Geo Batch.
     *
     * @param  \SplFileObject $geo_batch_file (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function apiV2GeoBatchUploadPostWithHttpInfo($geo_batch_file = null)
    {
        $request = $this->apiV2GeoBatchUploadPostRequest($geo_batch_file);

        try {
            $options = $this->createHttpClientOption();
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
                    $e->getResponse() ? $e->getResponse()->getHeaders() : null,
                    $e->getResponse() ? (string) $e->getResponse()->getBody() : null
                );
            }

            $statusCode = $response->getStatusCode();

            if ($statusCode < 200 || $statusCode > 299) {
                throw new ApiException(
                    sprintf(
                        '[%d] Error connecting to the API (%s)',
                        $statusCode,
                        $request->getUri()
                    ),
                    $statusCode,
                    $response->getHeaders(),
                    $response->getBody()
                );
            }

            $responseBody = $response->getBody();
            switch($statusCode) {
                case 200:
                    if ('\PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
                $content = (string) $responseBody;
            }

            return [
                ObjectSerializer::deserialize($content, $returnType, []),
                $response->getStatusCode(),
                $response->getHeaders()
            ];

        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
            }
            throw $e;
        }
    }

    /**
     * Operation apiV2GeoBatchUploadPostAsync
     *
     * Uploads file to Geo Batch.
     *
     * @param  \SplFileObject $geo_batch_file (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchUploadPostAsync($geo_batch_file = null)
    {
        return $this->apiV2GeoBatchUploadPostAsyncWithHttpInfo($geo_batch_file)
            ->then(
                function ($response) {
                    return $response[0];
                }
            );
    }

    /**
     * Operation apiV2GeoBatchUploadPostAsyncWithHttpInfo
     *
     * Uploads file to Geo Batch.
     *
     * @param  \SplFileObject $geo_batch_file (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoBatchUploadPostAsyncWithHttpInfo($geo_batch_file = null)
    {
        $returnType = '\PhoneBurner\AvalaraAFC\Model\GeoBatchSubmitFileResponse';
        $request = $this->apiV2GeoBatchUploadPostRequest($geo_batch_file);

        return $this->client
            ->sendAsync($request, $this->createHttpClientOption())
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, $returnType, []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                },
                function ($exception) {
                    $response = $exception->getResponse();
                    $statusCode = $response->getStatusCode();
                    throw new ApiException(
                        sprintf(
                            '[%d] Error connecting to the API (%s)',
                            $statusCode,
                            $exception->getRequest()->getUri()
                        ),
                        $statusCode,
                        $response->getHeaders(),
                        $response->getBody()
                    );
                }
            );
    }

    /**
     * Create request for operation 'apiV2GeoBatchUploadPost'
     *
     * @param  \SplFileObject $geo_batch_file (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function apiV2GeoBatchUploadPostRequest($geo_batch_file = null)
    {

        $resourcePath = '/api/v2/geo/batch/Upload';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        $multipart = false;




        // form params
        if ($geo_batch_file !== null) {
            $multipart = true;
            $formParams['geoBatchFile'] = \GuzzleHttp\Psr7\try_fopen(ObjectSerializer::toFormValue($geo_batch_file), 'rb');
        }
        // body params
        $_tempBody = null;

        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(
                ['text/plain', 'application/json', 'text/json']
            );
        } else {
            $headers = $this->headerSelector->selectHeaders(
                ['text/plain', 'application/json', 'text/json'],
                ['multipart/form-data']
            );
        }

        // for model (json/xml)
        if (isset($_tempBody)) {
            // $_tempBody is the method argument, if present
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($_tempBody));
            } else {
                $httpBody = $_tempBody;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    $multipartContents[] = [
                        'name' => $formParamName,
                        'contents' => $formParamValue
                    ];
                }
                // for HTTP post (form)
                $httpBody = new MultipartStream($multipartContents);

            } elseif ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \GuzzleHttp\Psr7\build_query($formParams);
            }
        }


        $defaultHeaders = [];
        if ($this->config->getUserAgent()) {
            $defaultHeaders['User-Agent'] = $this->config->getUserAgent();
        }

        $headers = array_merge(
            $defaultHeaders,
            $headerParams,
            $headers
        );

        $query = \GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'POST',
            $this->config->getHost() . $resourcePath . ($query ? "?{$query}" : ''),
            $headers,
            $httpBody
        );
    }

    /**
     * Operation apiV2GeoGeocodePost
     *
     * Geocodes one or multiple street addresses and/or lat/long coordinate pairs.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\GeocodeRequest[] $geocode_request List of street addresses and/or lat/long coordinate pairs to geocode. (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \PhoneBurner\AvalaraAFC\Model\GeocodeResult[]
     */
    public function apiV2GeoGeocodePost($geocode_request = null)
    {
        list($response) = $this->apiV2GeoGeocodePostWithHttpInfo($geocode_request);
        return $response;
    }

    /**
     * Operation apiV2GeoGeocodePostWithHttpInfo
     *
     * Geocodes one or multiple street addresses and/or lat/long coordinate pairs.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\GeocodeRequest[] $geocode_request List of street addresses and/or lat/long coordinate pairs to geocode. (optional)
     *
     * @throws \PhoneBurner\AvalaraAFC\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \PhoneBurner\AvalaraAFC\Model\GeocodeResult[], HTTP status code, HTTP response headers (array of strings)
     */
    public function apiV2GeoGeocodePostWithHttpInfo($geocode_request = null)
    {
        $request = $this->apiV2GeoGeocodePostRequest($geocode_request);

        try {
            $options = $this->createHttpClientOption();
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
                    $e->getResponse() ? $e->getResponse()->getHeaders() : null,
                    $e->getResponse() ? (string) $e->getResponse()->getBody() : null
                );
            }

            $statusCode = $response->getStatusCode();

            if ($statusCode < 200 || $statusCode > 299) {
                throw new ApiException(
                    sprintf(
                        '[%d] Error connecting to the API (%s)',
                        $statusCode,
                        $request->getUri()
                    ),
                    $statusCode,
                    $response->getHeaders(),
                    $response->getBody()
                );
            }

            $responseBody = $response->getBody();
            switch($statusCode) {
                case 200:
                    if ('\PhoneBurner\AvalaraAFC\Model\GeocodeResult[]' === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\PhoneBurner\AvalaraAFC\Model\GeocodeResult[]', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\PhoneBurner\AvalaraAFC\Model\GeocodeResult[]';
            $responseBody = $response->getBody();
            if ($returnType === '\SplFileObject') {
                $content = $responseBody; //stream goes to serializer
            } else {
                $content = (string) $responseBody;
            }

            return [
                ObjectSerializer::deserialize($content, $returnType, []),
                $response->getStatusCode(),
                $response->getHeaders()
            ];

        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\PhoneBurner\AvalaraAFC\Model\GeocodeResult[]',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
            }
            throw $e;
        }
    }

    /**
     * Operation apiV2GeoGeocodePostAsync
     *
     * Geocodes one or multiple street addresses and/or lat/long coordinate pairs.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\GeocodeRequest[] $geocode_request List of street addresses and/or lat/long coordinate pairs to geocode. (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoGeocodePostAsync($geocode_request = null)
    {
        return $this->apiV2GeoGeocodePostAsyncWithHttpInfo($geocode_request)
            ->then(
                function ($response) {
                    return $response[0];
                }
            );
    }

    /**
     * Operation apiV2GeoGeocodePostAsyncWithHttpInfo
     *
     * Geocodes one or multiple street addresses and/or lat/long coordinate pairs.
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\GeocodeRequest[] $geocode_request List of street addresses and/or lat/long coordinate pairs to geocode. (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiV2GeoGeocodePostAsyncWithHttpInfo($geocode_request = null)
    {
        $returnType = '\PhoneBurner\AvalaraAFC\Model\GeocodeResult[]';
        $request = $this->apiV2GeoGeocodePostRequest($geocode_request);

        return $this->client
            ->sendAsync($request, $this->createHttpClientOption())
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
                        $content = $responseBody; //stream goes to serializer
                    } else {
                        $content = (string) $responseBody;
                    }

                    return [
                        ObjectSerializer::deserialize($content, $returnType, []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                },
                function ($exception) {
                    $response = $exception->getResponse();
                    $statusCode = $response->getStatusCode();
                    throw new ApiException(
                        sprintf(
                            '[%d] Error connecting to the API (%s)',
                            $statusCode,
                            $exception->getRequest()->getUri()
                        ),
                        $statusCode,
                        $response->getHeaders(),
                        $response->getBody()
                    );
                }
            );
    }

    /**
     * Create request for operation 'apiV2GeoGeocodePost'
     *
     * @param  \PhoneBurner\AvalaraAFC\Model\GeocodeRequest[] $geocode_request List of street addresses and/or lat/long coordinate pairs to geocode. (optional)
     *
     * @throws \InvalidArgumentException
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function apiV2GeoGeocodePostRequest($geocode_request = null)
    {

        $resourcePath = '/api/v2/geo/Geocode';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        $multipart = false;




        // body params
        $_tempBody = null;
        if (isset($geocode_request)) {
            $_tempBody = $geocode_request;
        }

        if ($multipart) {
            $headers = $this->headerSelector->selectHeadersForMultipart(
                ['text/plain', 'application/json', 'text/json']
            );
        } else {
            $headers = $this->headerSelector->selectHeaders(
                ['text/plain', 'application/json', 'text/json'],
                ['application/json-patch+json', 'application/json', 'text/json', 'application/_*+json']
            );
        }

        // for model (json/xml)
        if (isset($_tempBody)) {
            // $_tempBody is the method argument, if present
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($_tempBody));
            } else {
                $httpBody = $_tempBody;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    $multipartContents[] = [
                        'name' => $formParamName,
                        'contents' => $formParamValue
                    ];
                }
                // for HTTP post (form)
                $httpBody = new MultipartStream($multipartContents);

            } elseif ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \GuzzleHttp\Psr7\build_query($formParams);
            }
        }


        $defaultHeaders = [];
        if ($this->config->getUserAgent()) {
            $defaultHeaders['User-Agent'] = $this->config->getUserAgent();
        }

        $headers = array_merge(
            $defaultHeaders,
            $headerParams,
            $headers
        );

        $query = \GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'POST',
            $this->config->getHost() . $resourcePath . ($query ? "?{$query}" : ''),
            $headers,
            $httpBody
        );
    }

    /**
     * Create http client option
     *
     * @throws \RuntimeException on file opening failure
     * @return array of http client options
     */
    protected function createHttpClientOption()
    {
        $options = [];
        if ($this->config->getDebug()) {
            $options[RequestOptions::DEBUG] = fopen($this->config->getDebugFile(), 'a');
            if (!$options[RequestOptions::DEBUG]) {
                throw new \RuntimeException('Failed to open the debug file: ' . $this->config->getDebugFile());
            }
        }

        return $options;
    }
}
